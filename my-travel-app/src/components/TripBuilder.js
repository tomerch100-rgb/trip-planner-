...
const TripBuilder = ({ savedAttractions, destinationName }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
          ...
          {savedAttractions?.length === 0 ? (
            <p className="text-sm text-gray-500">עדיין לא הוספת אטרקציות.</p>
          ) : (
        ...
        <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          {itinerary.length === 0 ? (
            <div className="min-h-[150px] flex items-center justify-center border-2 border-dashed border-gray-300 rounded bg-white">
              <p className="text-gray-400 text-sm">לחץ על "שבץ בלו"ז" כדי לבנות את המסלול...</p>
            </div>
          ) : (
            <ul className="space-y-3">
              {itinerary.map((item, index) => (
                <li key={index} className="bg-white p-3 rounded shadow-sm border border-gray-300 flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    ...
                    <button onClick={() => handleRemove(item.name)} className="text-red-500 hover:text-red-700 font-bold px-2">✕</button>
                  </div>
                  <div className="flex gap-4 text-sm bg-gray-50 p-2 rounded border border-gray-100">
                    <div className="flex flex-col w-1/2">
                      ...
                      <select value={item.day_number} onChange={(e) => handleChangeSchedule(index, 'day_number', parseInt(e.target.value))} className="border border-gray-300 rounded px-2 py-1 bg-white outline-none">
                    ...
                    <div className="flex flex-col w-1/2">
                      ...
                      <input type="time" value={item.start_time} onChange={(e) => handleChangeSchedule(index, 'start_time', e.target.value)} className="border border-gray-300 rounded px-2 py-1 outline-none" />
                    </div>
                  </div>
                </li>
              ))}
      ...
      {/* המפה - מופיעה אוטומטית כשיש אטרקציות בלו"ז */}
      {itinerary.length > 0 && (
        <div className="mt-8 border-t pt-8">
          <h3 className="font-bold text-gray-800 mb-4 text-lg">מפת המסלול שלך</h3>
      ...
      <div className="mt-8 text-left border-t pt-6">
        <button 
          ...
          className="bg-green-600 text-white px-8 py-3 rounded-lg font-bold shadow hover:bg-green-700 transition disabled:bg-gray-400"
        >
          שמור מסלול סופי
        </button>
      </div>
